...
class TestSummary(unittest.TestCase):
    ...
    def _copy_page_kwargs(self):
        ...
        page_kwargs = dict([(key, self.page_kwargs[key]) for key in self.page_kwargs])
        for key in page_kwargs:
            if not isinstance(page_kwargs[key], dict):
                break
            page_kwargs[key] = dict(
    ...
    def test_begin_summary(self):
        page_kwargs = self._copy_page_kwargs()
        del page_kwargs["metadata"]["summary"]
        page_kwargs["content"] = "FOOBAR<!-- PELICAN_BEGIN_SUMMARY -->" + TEST_CONTENT
        page = Page(**page_kwargs)
        summary.extract_summary(page)
        # test both the summary and the marker removal
    ...
    def test_correct_malformed_markup(self):
        ...
        wellformed = (
            "<article><div><h2>Title</h2>" "<p>Some content</p></div></article>"
        )
        page_kwargs["content"] = (
