...
class HTMLTranslator(PelicanHTMLTranslator):
    ...
    def visit_label_info(self, node):
        self.body.append('<span class="label label-info">%s</span>' % node.children[0])
        raise nodes.SkipNode
    ...
    def visit_progress(self, node):
        ...
        self.body.append('<div class="%s" %s>%s</div>' % (classes, properties, label))
        self.body.append("</div>")
        raise nodes.SkipNode
    ...
    def visit_sidebar(self, node):
        ...
        self.body.append(
            self.starttag(node, "div", CLASS="bs-docs-sidebar hidden-print affix-top")
        )
