...
class TestPelicanfly(unittest.TestCase):
    ...
    def test_add_markdown_plugin(self):
        added = any(
            [
                isinstance(x, FontAwesomeExtension)
    ...
    def test_markdown_plugin(self):
        sample_output = open(
            os.path.join(self.temp_path, "pages", "a-sample-page.html"), "r"
        )
    ...
    def test_assets_exist(self):
        for static_dir in ["css", "fonts"]:
            ...
            for static_file in os.listdir(static_path):
                in_theme = os.path.join(
                    self.temp_path, "theme", static_dir, static_file
                )
