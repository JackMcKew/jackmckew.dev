...
def get_subcategories(generator, metadata):
    ...
    if (
        "PAGINATED_TEMPLATES" in generator.settings
        and "subcategory" not in generator.settings["PAGINATED_TEMPLATES"]
    ):
...
def create_subcategories(generator):
    ...
    for article in generator.articles:
        ...
        for subcategory in article.subcategories:
            ...
            sub_cat = [
                item for item in generator.subcategories if item[0].name == subcategory
            ]
            if sub_cat:
...
def generate_subcategories(generator, writer):
    write = partial(
        writer.write_file, relative_urls=generator.settings["RELATIVE_URLS"]
    )
    ...
    for subcat, articles in generator.subcategories:
        ...
        write(
            ...
            articles=articles,
            dates=dates,
            template_name="subcategory",
            page_name=subcat.page_name,
    ...
    for subcat, articles in generator.subcategories:
        ...
        if generator.settings.get("SUBCATEGORY_FEED_ATOM"):
            writer.write_feed(
                articles,
                generator.context,
                generator.settings["SUBCATEGORY_FEED_ATOM"] % subcat.fullurl,
            )
        if generator.settings.get("SUBCATEGORY_FEED_RSS"):
            writer.write_feed(
                articles,
                generator.context,
                generator.settings["SUBCATEGORY_FEED_RSS"] % subcat.fullurl,
                feed_type="rss",
            )
