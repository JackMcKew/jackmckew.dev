  ...
  """
    ...
    """
        ...
        for i, xml in enumerate(xml_photos):
            ...
            logger.debug(
                u"%d/%d %s from %s"
                % (i + 1, len(xml_photos), photo, photo.cached and "cache" or "flickr")
            )
...
class FlickrPhoto(FlickrCached):
    ...
    def load_from_flickr(self):
        ...
        try:
            ...
            if (
                not self.data
                or infos["dates"]["lastupdate"]
                != self.data["infos"]["dates"]["lastupdate"]
            ):
                self.cached = False
                self.data = {
                    "infos": infos,
    ...
    def __unicode__(self):
        return u"Flickr Photo %s '%s'" % (
            self.id,
            "infos" in self.data and self.data["infos"]["title"] or "-",
        )
