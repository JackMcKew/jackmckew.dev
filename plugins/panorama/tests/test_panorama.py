...
from panorama.data_factory import (
...
class TestGenerator(unittest.TestCase):
    ...
    def tearDown(self):
        ...
        with io.open(
            os.path.join(TEST_DIR, "all_charts.html"), "w", encoding="utf8"
        ) as output_file:
            output_file.write(
                self.template_test_page.render(
                    panorama_charts=self.generator.context["panorama_charts"]
                )
            )
...
class TestData(unittest.TestCase):
    ...
    def test_count_article_by_column_by_year(self):
        self.assertEqual(
            len(count_article_by_column_by_year(self.data_factory.data, "genre")), 3
        )

    def test_count_article_by_column(self):
        expected_result = Series(
            {"BD": 4, "Divers": 1, "Jeunesse": 1, "Roman": 3, "Roman Noir": 1}
        )
        print(count_article_by_column(self.data_factory.data, "genre"))
        print(expected_result)
        assert_series_equal(
            count_article_by_column(self.data_factory.data, "genre"),
            expected_result,
            check_names=False,
        )
    ...
    def test_count_article_by_year(self):
        ...
        assert_series_equal(
            count_article_by_year(self.data_factory.data),
            expected_result,
            check_names=False,
        )
    ...
    def test_top_article(self):
        ...
        assert_series_equal(
            top_article(self.data_factory.data, "publisher", 1),
            expected_result,
            check_names=False,
        )
...
class TestConf(unittest.TestCase):
    ...
    def test_chart_configuration(self):
        ...
        renderer = partial(
            self.chart_factory.get_renderer(class_name="pieChart"), name="pie_chart"
        )
        chart = self.chart_factory.render(data=data, renderer=renderer)
