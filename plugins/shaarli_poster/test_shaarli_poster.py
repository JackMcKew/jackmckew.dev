...
class ShaarliPosterTest(unittest.TestCase):
    @patch(
        "shaarli_poster.shaarli_poster.get_credentials"
    )  # avoid ~/.config/shaarli/client.ini parsing, which may not exist in CI
    ...
    def test_upload_new_article(self, ShaarliClientClassMock, _):
        ...
        with TemporaryDirectory() as tmpdirname:
            upload_new_articles(
                [
                    build_article_generator(
                        get_settings(filenames={}), TEST_CONTENT_DIR, tmpdirname
                    )
                ]
            )
            self.assertEqual(
                ShaarliClientClassMock.return_value.request.call_count, 2
            )  # 1 get-links + 1 post-link
