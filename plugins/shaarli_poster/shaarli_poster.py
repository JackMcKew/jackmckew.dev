...
def upload_new_articles(generators):
    ...
    articles_urls_on_shaarli = get_published_articles_urls(
        client, article_generator.settings
    )
    for article in article_generator.articles:
        if (
            article.status == "published"
            and article.url not in articles_urls_on_shaarli
        ):
...
def get_desc_and_tags(article, pelican_settings):
    tags = [tag.name for tag in article.tags] + [
        pelican_settings.get("SHAARLI_POSTER_TAG", DEFAULT_TAG)
    ]
    # We transform relative images URLs into absolute ones:
    description = re.sub(
        ' src="(?!http)',
        ' src="' + os.path.join(pelican_settings["SITEURL"], ""),
        article.summary,
    )
...
def main():
    ...
    args_parser.add_argument(
        "--all",
        action="store_true",
        help="Do not filter out articles already published",
    )
    ...
    articles_urls_on_shaarli = (
        set() if args.all else get_published_articles_urls(client, settings)
    )
    links = []
...
def small_hash(date, id):
    ...
    return (
        b64encode(php_crc32(text.encode()).to_bytes(4, "big")).decode().replace("=", "")
    )
