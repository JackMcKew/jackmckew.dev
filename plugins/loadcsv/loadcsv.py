...
def csv_loader(csv_elem, curpath, linefeed="\n", delim=",", classes=["table"], limit=5):
    ...
    if filename:
        csv_string += (
            '<tr><td colspan="%s" class="data-link"><a href="%s">data link</a></td></tr>'
            % (len(csv_list[0].split(delim)), filename)
        )
    csv_string += "</table>"
...
def loadcsv(data_passed_from_pelican):
    ...
    all_csv_elements = re.findall(
        "{% csv .*? %}", page_content, re.DOTALL
