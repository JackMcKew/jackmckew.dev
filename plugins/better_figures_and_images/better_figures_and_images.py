        ...
        for img in soup(["img", "object"]):
            ...
            if img_filename.startswith("{attach}"):
                ...
                image_output_location = path.join(
                    instance.settings["OUTPUT_PATH"], output_path, img_filename
                )
                if path.isfile(image_output_location):
                    src = image_output_location
                    logger.info(
                        "{src} located in output, missing from content.".format(
                            src=img_filename
                        )
                    )
                else:
                    logger.warning(
                        "Better Fig. Error: img_path should start with either {attach}, {filename}, |filename|, {static} or /static"
                    )
            ...
            if src is None:
                ...
                src_candidates += [
                    os.path.join(
                        instance.settings["PATH"], static_path, img_path, img_filename
                    )
                    for static_path in instance.settings["STATIC_PATHS"]
                ]

                src_candidates = [
                    f for f in src_candidates if path.isfile(f) and access(f, R_OK)
                ]

                if not src_candidates:
                    logger.error("Better Fig. Error: image not found: %s", src)
                    logger.debug(
                        "Better Fig. Skip src: %s", img_path + "/" + img_filename
                    )
                    continue
            ...
            if (
                "RESPONSIVE_IMAGES" in instance.settings
                and instance.settings["RESPONSIVE_IMAGES"]
            ):
