...
class Tipue_Search_JSON_Generator(object):
    ...
    def create_json_node(self, page):
        ...
        page_category = (
            page.category.name if getattr(page, "category", "None") != "None" else ""
        )

        page_url = "."
        if page.url:
            page_url = (
                page.url if self.relative_urls else (self.siteurl + "/" + page.url)
            )
        ...
        node = {
            ...
            "url": page_url,
            "loc": page_url,
        }  # changed from 'url' following http://blog.siphos.be/2015/08/updates-on-my-pelican-adventure/ (an update to Pelican made it not work, because the update (e.g., in the theme folder, static/tipuesearch/tipuesearch.js is looking for the 'loc' attribute.
    ...
    def create_tpage_node(self, srclink):
        ...
        srcfile = open(
            os.path.join(self.output_path, self.tpages[srclink]), encoding="utf-8"
        )
