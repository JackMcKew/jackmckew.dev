...
"""
    ...
    def testKnitrSettings(self):
        ...
        self.assertTrue(
            os.path.exists(outputfilename), "File %s was not created." % outputfilename
        )
        ...
        self.assertTrue(
            len(images) == 1,
            "Contents of images dir is not correct: %s" % ",".join(images),
        )
    ...
    def testKnitrSettings2(self):
        ...
        imagefile = (
            os.path.join(
                imagesdir, os.path.splitext(os.path.split(self.contentfile)[1])[0]
            )
            + "-1-1.png"
        )
        logging.debug(imagefile)
