...
def prepare_site_db_and_overrides():
    ...
    for lang, overrides in _SUBSITE_QUEUE.items():
        ...
        if "OUTPUT_PATH" not in overrides:
            overrides["OUTPUT_PATH"] = os.path.join(_MAIN_SETTINGS["OUTPUT_PATH"], lang)
        if "CACHE_PATH" not in overrides:
            overrides["CACHE_PATH"] = os.path.join(_MAIN_SETTINGS["CACHE_PATH"], lang)
        if "STATIC_PATHS" not in overrides:
            overrides["STATIC_PATHS"] = []
        if (
            "THEME" not in overrides
...
class GeneratorInspector(object):
    ...
    def __init__(self, generator):
        ...
        self.generators_info.update(generator.settings.get("I18N_GENERATORS_INFO", {}))
    ...
    def translations_lists(self):
        ...
        return (
            ...
            for name in self.info.get("translations_lists", [])
        )

    def contents_list_pairs(self):
        """Iterator over pairs of normal and hidden contents"""
        return (
            tuple(getattr(self.generator, name) for name in names)
...
def create_next_subsite(pelican_obj):
    ...
    if len(_SUBSITE_QUEUE) == 0:
        ...
        with temporary_locale():
            ...
            _LOGGER.debug(
                ("Generating i18n subsite for language '{}' " "using class {}").format(
                    lang, cls
                )
            )
            new_pelican_obj.run()
