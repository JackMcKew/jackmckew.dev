        ...
        if "gallery" in article.metadata.keys():
            ...
            if os.path.isdir(articlegallerypath):
                for i in os.listdir(articlegallerypath):
                    if not i.startswith(".") and os.path.isfile(
                        os.path.join(os.path.join(gallerycontentpath, album), i)
                    ):
    ...
    for page in generator.pages:
        if page.metadata.get("template") == "gallery":
            ...
            for a in os.listdir(gallerycontentpath):
                if not a.startswith(".") and os.path.isdir(
                    os.path.join(gallerycontentpath, a)
                ):

                    for i in os.listdir(os.path.join(gallerycontentpath, a)):
                        if not a.startswith(".") and os.path.isfile(
                            os.path.join(os.path.join(gallerycontentpath, a), i)
                        ):
