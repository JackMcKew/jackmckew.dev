...
def extract_toc(content):
    ...
    if (
        not toc
        and readers.MarkdownReader.enabled
        and extension in readers.MarkdownReader.file_extensions
    ):
    ...
    if (
        not toc
        and readers.RstReader.enabled
        and extension in readers.RstReader.file_extensions
    ):
    ...
    if "pandoc_reader" in content.settings["PLUGINS"]:
        ...
        if (
            not toc
            and PandocReader
            and PandocReader.enabled
            and extension in PandocReader.file_extensions
        ):
