...
@pytest.mark.parametrize(
    ...
    [
        (
            ...
            dict(photo_id="18873146680", size="large", alt="test 1"),
        ),
        (
            "18873146680 large 'test 1'",
            dict(photo_id="18873146680", size="large", alt="test 1"),
        ...
        (
            ...
            dict(photo_id="18873143536360", size="medium", alt="test number two"),
        ),
        (
            '18873143536360 small "test number 3"',
            dict(photo_id="18873143536360", size="small", alt="test number 3"),
        ),
        (
            '18873143536360 "test 4"',
            dict(photo_id="18873143536360", size=None, alt="test 4"),
        ),
        ("18873143536360", dict(photo_id="18873143536360", size=None, alt=None)),
        ("123456 small", dict(photo_id="123456", size="small", alt=None)),
    ],
)
...
def test_source_url(input, expected):
    assert (
        flickr.source_url(input[0], input[1], input[2], input[3], input[4]) == expected
    )
...
def test_generage_html(mock_urlopen):
    ...
    with open(TEST_DATA_DIR + "/flickr.json", "rb") as f:
        ...
        attrs = dict(photo_id="1234567", size="large", alt="this is a test")
