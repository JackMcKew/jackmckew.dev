...
class VocabularyGenerator(CachingGenerator):
    ...
    def generate_vocabulary_context(self, vocabulary_file_name, path_to_vocabulary):
        logger.debug(
            "Generating__ vocabulary context for "
            + path_to_vocabulary
            + "/"
    ...
    def generate_local_context(self):
        for f in self.get_files(
            self.settings["VOC_PATHS"], exclude=self.settings["VOC_EXCLUDES"]
        ):
    ...
    def generate_output(self, writer):
        for voc in self.vocabularies:
            writer.write_file(
                voc.save_as,
                self.get_template(voc.template),
                self.context,
                voc=voc,
...
class RdfReader(BaseReader):
    ...
    def read(self, source_path):
        ...
        queries = [
            f
            for f in listdir(self.settings["VOC_QUERIES_PATH"])
            if (
                isfile(join(self.settings["VOC_QUERIES_PATH"], f))
