...
def generate_screenshot(themesroot, theme):
    ...
    os.system(
        "CutyCapt %s --url=http://localhost:8000 --out=content/static/%s_index_tmp.png"
        % (params, theme)
    )
    os.system(
        "CutyCapt %s --url=http://localhost:8000/details-information-for-%s-theme.html --out=content/static/%s_article_tmp.png"
        % (params, theme, theme)
    )
    os.system(
        "convert content/static/%s_index_tmp.png -trim content/static/%s_index.png"
        % (theme, theme)
    )
    os.system(
        "convert content/static/%s_article_tmp.png -trim content/static/%s_article.png"
        % (theme, theme)
    )
    os.system(
        "convert content/static/%s_index.png -resize %s -gravity north -crop 410x307 content/static/small_%s_index.png"
        % (theme, "410x307", theme)
    )
    os.system(
        "convert content/static/%s_article.png -resize %s  -gravity north -crop 410x307 content/static/small_%s_article.png"
        % (theme, "410x307", theme)
    )
