        ...
        def write_file(
            ...
            **kwargs
            ...
            if paginated:
                ...
                paginators = {
                    key: Paginator(name, val, self.settings)
                ...
                for page_num in range(list(paginators.values())[0].num_pages):
                    ...
                    for key in paginators.keys():
                        ...
                        previous_page = (
                            paginator.page(page_num) if page_num > 0 else None
                        )
...
def register():
    ...
    if not plim:
        logger.warning(
            "`slim` failed to load dependency `plim`. " "`slim` plugin not loaded."
        )
        return
    if not mako:
        logger.warning(
            "`slim` failed to load dependency `mako`. " "`slim` plugin not loaded."
        )
        return
    if not bs:
        logger.warning(
    ...
    if not minify:
        logger.warning(
            "`slim` failed to load dependency `htmlmin`. " "`slim` plugin not loaded."
        )
        return
