...
class PlantUMLBlockProcessor(markdown.blockprocessors.BlockProcessor):
    ...
    def run(self, parent, blocks):
        ...
        imageurl = (
            self.config["siteurl"] + "/images/" + generate_uml_image(path, text, format)
        )
        # Create image tag and append to the document
        etree.SubElement(
            parent, "img", src=imageurl, alt=alt, attrib={"class": classes}
        )
...
class PlantUMLMarkdownExtension(markdown.Extension):
    ...
    def __init__(self, *args, **kwargs):
        self.config = {
            "classes": [
                "uml",
                "Space separated list of classes for the generated image. Default uml.",
            ],
