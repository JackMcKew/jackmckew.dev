...
def pretaxonomy_hook(generator):
    ...
    for article in generator.articles:
        ...
        if fname == "index":
            category_objects[dirname] = make_category(
                article, os.path.basename(dirname)
            )
        else:
            real_articles.append(article)

    category_assignment = re.compile(
        "^(" + "|".join(re.escape(prefix) for prefix in category_objects.keys()) + ")/"
    )
    ...
    for article in real_articles:
        ...
        if not m or m.group(1) not in category_objects:
            logger.error(
                "No category assignment for %s (%s)", article, article.source_path
            )
            continue
