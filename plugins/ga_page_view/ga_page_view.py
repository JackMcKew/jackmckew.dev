...
def get_service(api_name, api_version, scopes, key_file_location):
    try:
        ...
        sys.stderr.write(
            "[ga_page_view] Failed to fetch page view information. "
            + ExceptionDetails()
        )
        return None
...
def get_page_view(generators):
    try:
        ...
        popular_start_date = str(
            pdt.Calendar().parseDT(popular_start_str, datetime.datetime.now())[0].date()
        )
        popular_result = (
            service.data()
            .ga()
        ...
        total_page_view = 0
        for pages, url_pattern in [
            (article_generator.articles, ARTICLE_SAVE_AS),
        ...
        sys.stderr.write(
            "[ga_page_view] Failed to fetch page view information." + ExceptionDetails()
        )
        # print(e)
