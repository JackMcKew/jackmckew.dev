...
logger = logging.getLogger(
    __name__
)  # For using logger.debug() to log errors or other notes.
...
from . import (
    video_service_thumbnail_url_generating_functions as video_thumbnail_functions,
)  # These are functions defined in 'video_service_thumbnail_url_generating_functions.py', which is located in the same directory as this file.
...
def check_for_thumbnail_directory(pelican_output_path):
    ...
    try:
        if not os.path.exists(
            pelican_output_path + "/" + output_directory_for_thumbnails
        ):  # If the directory doesn't exist already...
            os.makedirs(
                pelican_output_path + "/" + output_directory_for_thumbnails
            )  # Create the directory to hold the video thumbnails.
            return True
    except Exception as e:
        logger.error(
            "Video Privacy Enhancer Plugin: Error in checking if thumbnail folder exists and making the directory if it doesn't: %s",
            e,
...
def download_thumbnail(
    ...
    if not os.path.exists(
        pelican_output_path
        + "/"
        + output_directory_for_thumbnails
        + "/"
        + video_service_name
        + "_"
        + video_id_from_shortcode
        ...
        logger.debug(
            "Video Privacy Enhancer Plugin: Downloading thumbnail from the following url: "
            + video_thumbnail_url
        )
        six.moves.urllib.request.urlretrieve(
            ...
            + video_id_from_shortcode
            + ".jpg",
        )  # Download the thumbnail. This follows the instructions at http://www.reelseo.com/youtube-thumbnail-image/ for downloading YouTube thumbnail images.
...
def process_shortcodes(data_passed_from_pelican):
    ...
    if (
        not data_passed_from_pelican._content
    ...
    for video_service_name, video_service_information in six.iteritems(
        dictionary_of_services
    ):

        # Good for debugging:
        logger.debug(
            "Video Privacy Enhancer Plugin: The name of the current service being processed is '"
            + video_service_name
            + "'"
        )

        shortcode_to_search_for_not_including_exclamation_point = video_service_information[
            "shortcode_not_including_exclamation_point"
        ]
        logger.debug(
            "Video Privacy Enhancer Plugin: Currently looking for the shortcode '"
            + shortcode_to_search_for_not_including_exclamation_point
            + "'"
        )

        function_for_generating_thumbnail_url = video_service_information[
            "function_for_generating_thumbnail_url"
        ]

        all_instances_of_the_shortcode = re.findall(
            r"(?<!\\\)\!"
            + shortcode_to_search_for_not_including_exclamation_point
            + r".*?\)",
            data_passed_from_pelican._content,
        )  # Use a regular expression to find every instance of, e.g., '!youtube' followed by anything up to the first matching ')'.

        if (
            len(all_instances_of_the_shortcode) > 0
        ):  # If the article/page HAS any shortcodes, go on. Otherwise, don't (to do so would inadvertantly wipe out the output content for that article/page).
            ...
            for shortcode_to_parse in all_instances_of_the_shortcode:
                ...
                video_id_from_shortcode = re.findall(
                    "(?<="
                    + shortcode_to_search_for_not_including_exclamation_point
                    + r"\().*?(?=\))",
                    shortcode_to_parse,
                    ...
                    0
                ]  # Get what's inside of the parentheses in, e.g., '!youtube(...).'
                ...
                video_thumbnail_url = function_for_generating_thumbnail_url(
                    video_id_from_shortcode
                )
                ...
                replace_shortcode_in_text = re.sub(
                    r"\!"
                    + shortcode_to_search_for_not_including_exclamation_point
                    + r"\("
                    + video_id_from_shortcode
                    + r"\)",
                    r'<img class="video-embed-dummy-image" id="'
                    + video_id_from_shortcode
                    + '" src="'
                    + pelican_site_url
                    + "/"
                    ...
                    + "_"
                    + video_id_from_shortcode
                    + '.jpg" alt="Embedded Video - Click to view" title="Embedded Video - Click to view" embed-service="'
                    + video_service_name
                    + '"></img>',
                    data_passed_from_pelican._content,
                )
