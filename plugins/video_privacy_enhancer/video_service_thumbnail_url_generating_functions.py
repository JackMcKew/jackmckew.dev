...
def generate_thumbnail_download_link_youtube(video_id_from_shortcode):
    ...
    thumbnail_download_link = (
        "https://img.youtube.com/vi/" + video_id_from_shortcode + "/0.jpg"
    )
...
def generate_thumbnail_download_link_vimeo(video_id_from_shortcode):
    ...
    video_metadata = urlopen(
        "https://vimeo.com/api/v2/video/" + str(video_id_from_shortcode) + ".json"
    ).read()  # Download the video's metadata in JSON format.
    video_metadata_parsed = json.loads(video_metadata.decode("utf-8"))  # Parse the JSON
    video_thumbnail_large_location = video_metadata_parsed[0][
        "thumbnail_large"
