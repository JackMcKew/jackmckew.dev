...
def get_api_urls(settings):
    ...
    if api_key and urlname:
        meetup_group_url = "https://api.meetup.com/{}?photo_host=public&key={}".format(
            urlname, api_key
        )
        meetup_events_url = "https://api.meetup.com/{}/events?photo-host=public&page=20&key={}".format(
            urlname, api_key
        )
    else:
    ...
    if not (meetup_group_url and meetup_events_url):
        raise MeetupInfoPluginError(
            "Either (MEETUP_API_KEY and MEETUP_URLNAME) or "
