...
def encrypt_text_aes(text, password):
    ...
    plaintext_padded = (
        plaintext + (BLOCK_SIZE - len(plaintext) % BLOCK_SIZE) * PADDING_CHAR
    )
    ciphertext = cipher.encrypt(plaintext_padded)

    return (base64.b64encode(iv), base64.b64encode(ciphertext), PADDING_CHAR)
