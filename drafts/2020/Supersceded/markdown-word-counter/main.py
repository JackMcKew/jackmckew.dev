...
def count_words_in_jupyter(filePath: str, returnType: str = "markdown"):
    ...
    for cell in nb.worksheets[0].cells:
        if cell.cell_type == "markdown":
            word_count_markdown += len(
                cell["source"].replace("#", "").lstrip().split(" ")
            )
        elif cell.cell_type == "heading":
            word_count_heading += len(
                cell["source"].replace("#", "").lstrip().split(" ")
            )
...
topFolder: pathlib.Path = pathlib.Path(
    "C:\\Users\\jackm\\Documents\\GitHub\\jackmckew.dev\\content\\"
)
...
textStatistics["Word Count"] = textStatistics["Content"].apply(
